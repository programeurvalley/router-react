...
function Filter(){
    ...
    let data = movielist.filter((title , rating) =>  {
    ...
    let {title,description,url,rating} = inputdata

    function EntreObjectToArray(){
        (inputdata.title == "" || inputdata.description == "" || inputdata.url == "" || inputdata.rating == "") ? alert("All the field must be full") :
        setMovielist([...movielist,{title,description,url,rating}])
        setInputdata({title:"",description:"",url:"",rating:""})
    }
